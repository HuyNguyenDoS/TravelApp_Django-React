...
function Blogs(props) {
    ...
    // useEffect(() => {
    //     let loadNewestArticals = async () => {
    //         try {
    //             let res = await API.get(endpoints['newest-articals'])
    //             setLastestArticals(res.data)
    //         } catch (error) {
    //             console.error(error)
    //         }
    //     }
...
class ArticalItem extends React.Component {
    componentDidMount() {
        new WOW.WOW({
            live: false
        }).init();
                ...
                <div className="news-block-one wow fadeInUp animated animated" data-wow-delay="00ms" data-wow-duration="1500ms">
                    <div className="inner-box">
                        <figure className="image-box">
                            <Link to={"/artical-details/" + this.props.artical.id}>
                                <img style={{width: "770px", height: "470px"}} src={this.props.artical.image} alt="ImageBlog"/>
                            </Link>
                            <span className="post-date">
                                <i className="far fa-calendar-alt" />
                                {this.props.artical.created_date}
                            </span>
                        </figure>
                        <div className="lower-content">
                            <div className="category">
                                <Link to={"/artical-details/" + this.props.artical.id}>Lifestyle</Link>
                            </div>
                            <h2>
                                <Link to={"/artical-details/" + this.props.artical.id}>
                                    {this.props.artical.title}
                                </Link>
                            </h2>
                            <ul className="post-info clearfix">
                                <li>
                                    <span>Theo</span> <Link to={"/artical-details/" + this.props.artical.id}>{this.props.artical.author}</Link>
                                </li>
                                <li> - {this.props.artical.created_date}</li>
                            </ul>
                            <div className="btn-box">
                                <Link to={"/artical-details/" + this.props.artical.id} className="theme-btn-two">
                                    Xem chi tiết
                                </Link>
                            </div>
                        </div>
